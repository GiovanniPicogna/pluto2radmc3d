...
def read_parameters_file(parameters):
    ...
    # bin_small_dust
    default = "No"
    parameters['bin_small_dust'] = str(read_parameter('bin_small_dus'
                                                      't', default))
...
def print_parameters(parameters):
    ...
    print('do we use pre-calculated opacities, which are located in opacity_dir directory? : ',
    ...
    print('do we recompute fits file of raw flux map from image.out? This is useful if radmc has been run on a /'
          'different platform: ', parameters['recalc_rawfits'])
    ...
    if parameters['calc_abs_map'] == 'Yes':
        print('if so, do we assume the dust surface density equal to the gas surface density? : ',
    ...
    print('do we compute the axisymmetric profile of the convolved intensity?:', parameters['axi_intensity'])
    ...
    if parameters['RTdust_or_gas'] == 'dust':
        parameters['label'] = parameters['dir']+'_o'+str(parameters['on'])+'_p'+str(parameters['pindex'])+'_r'+str(parameters['ratio'])+'_a'+str(parameters['amin'])+'_'+str(parameters['amax'])+'_nb'+str(parameters['nbin'])+'_mode'+str(parameters['scat_mode'])+'_np'+str('{:.0e}'.format(parameters['nb_photons']))+'_nc'+str(parameters['nsec'])+'_z'+str(parameters['z_expansion'])+'_xf'+str(parameters['xaxisflip'])+'_Td'+str(parameters['Tdust_eq_Thydro'])
    else:
        if parameters['widthkms'] == 0.0:
            parameters['label'] = parameters['dir']+'_o'+str(parameters['on'])+'_gas'+str(parameters['gasspecies'])+'_iline'+str(parameters['iline'])+'_lmode'+str(parameters['lines_mode'])+'_ab'+str('{:.0e}'.format(parameters['abundance']))+'_vkms'+str(parameters['vkms'])+'_turbvel'+str(parameters['turbvel'])+'_nc'+str(parameters['nsec'])+'_xf'+str(parameters['xaxisflip'])+'_Td'+str(parameters['Tdust_eq_Thydro'])
        else:
            parameters['label'] = parameters['dir']+'_o'+str(parameters['on'])+'_gas'+str(parameters['gasspecies'])+'_iline'+str(parameters['iline'])+'_lmode'+str(parameters['lines_mode'])+'_ab'+str('{:.0e}'.format(parameters['abundance']))+'_widthkms'+str(parameters['widthkms'])+'_turbvel'+str(parameters['turbvel'])+'_nc'+str(parameters['nsec'])+'_xf'+str(parameters['xaxisflip'])+'_Td'+str(parameters['Tdust_eq_Thydro'])
