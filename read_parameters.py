...
def read_parameters_file(parameters):
    ...
    # recalc_temperature
    default = "Yes"
    parameters['recalc_temperature'] = str(read_parameter('recalc_temperature',default))
    ...
    # bin_small_dust
    default = "No"
    parameters['bin_small_dust'] = str(read_parameter('bin_small_dust',default))
    ...
    # nb_photons_scat
    default = 3.e8
    parameters['nb_photons_scat'] = int(float(read_parameter('nb_photons_scat',default)))
    ...
    # truncation_radius
    default = 0.
    parameters['truncation_radius'] = float(read_parameter('truncation_radius',default))
    ...
    # kinematic viscosity has been used in the simulation
        ...
        parameters['alphaviscosity'] = viscosity * (aspectratio**(-2.0))
...
def check_parameters_consistency(parameters):
    ...
    if parameters['polarized_scat'] == 'Yes':
        parameters['z_expansion'] = 'G'
        if parameters['scat_mode'] != 5:
            sys.exit(
                "To get a polarized intensity image you need scat_mode = 5 in params.dat. Abort!")
...
def print_parameters(parameters):
    ...
    if parameters['RTdust_or_gas'] == 'gas':
        ...
        print('number of wavelengths for multi-color gas images ?', parameters['linenlam'])
    ...
    if parameters['polarized_scat'] == 'Yes':
        print(
            'truncation_radius for dust pressure scale height [arcseconds] : ', parameters['truncation_radius'])
        print(
            'mask_radius in polarized intensity image [arcseconds] : ', mask_radius)
    ...
    print('Do we display intensity in brightness temperature?: ', parameters['brightness_temp'])
    print('number of grid cells in phi for RADMC3D = ', parameters['nsec'])
    print('type of vertical expansion done for dust mass volume density = ', parameters['z_expansion'])
    ...
    print('do we recompute all dust or gas temperatures? : ', parameters['recalc_temperature'])
    print('do we include a bin with small dust tightly coupled to the gas? : ', parameters['bin_small_dust'])
    print('what is the maximum altitude of the 3D grid in pressure scale heights? : ', parameters['zmax_over_H'])
    print('do we recompute all dust opacities? : ', parameters['recalc_opac'])
    print('do we plot dust opacities? : ', parameters['plot_opac'])
    print('do we use pre-calculated opacities, which are located in opacity_dir directory? : ', parameters['precalc_opac'])
    print('do we run RADMC3D calculation of temperature and ray tracing? : ', parameters['recalc_radmc'])
    if parameters['recalc_radmc'] == 'Yes':
        print('how many cores do we use for radmc calculation? : ', parameters['nbcores'])
    print('do we recompute fits file of raw flux map from image.out? This is useful if radmc has been run on a different platform: ', parameters['recalc_rawfits'])
    print('do we recompute convolved flux map from the output of RADMC3D calculation? : ', parameters['recalc_fluxmap'])
    print('do we compute 2D analytical solution to RT equation w/o scattering? : ', parameters['calc_abs_map'])
    if parameters['calc_abs_map'] == 'Yes':
        print('if so, do we assume the dust surface density equal to the gas surface density? : ', parameters['dustdens_eq_gasdens'])
    print('do we take the gas (hydro) temperature for the dust temperature? : ', parameters['Tdust_eq_Thydro'])
    ...
    print('number of pixels in each direction for flux map computed by RADMC3D = ', parameters['nbpixels'])
    ...
    print('number of photon packaged used by RADMC3D for scattering', parameters['nb_photons_scat'])
    print('type of dust particles for calculation of dust opacity files = ', parameters['species'])
    print('name of directory with .lnk dust opacity files = ', parameters['opacity_dir'])
    print('x- and y-max in final image [arcsec] = ', parameters['minmaxaxis'])
    print('do we use second-order integration for ray tracing in RADMC3D? : ', parameters['secondorder'])
    print('do we flip x-axis in disc plane? (mirror symmetry x -> -x in the simulation): ', parameters['xaxisflip'])
    print('do we check beam shape by adding a source point at the origin?: ', parameters['check_beam'])
    print('do we deproject the predicted image into polar coords?:', parameters['deproj_polar'])  # SP
    print('do we compute the axisymmetric profile of the convolved intensity?:',
        parameters['axi_intensity'])  # GWF                                                                                                                                                                                                                                                                                                          = 'Yes'
    print('do we plot radial temperature profiles? : ', parameters['plot_temperature'])
    print('do we override code units in hydro simulation? : ', parameters['override_units'])
    if parameters['override_units'] == 'Yes':
    ...
    if parameters['RTdust_or_gas'] == 'dust':
        parameters['label'] = parameters['dir']+'_o'+str(parameters['on'])+'_p'+str(parameters['pindex'])+'_r'+str(parameters['ratio'])+'_a'+str(parameters['amin'])+'_'+str(parameters['amax'])+'_nb'+str(parameters['nbin'])+'_mode'+str(parameters['scat_mode']) + \
            '_np'+str('{:.0e}'.format(parameters['nb_photons']))+'_nc'+str(parameters['nsec'])+'_z' + \
            str(parameters['z_expansion'])+'_xf'+str(parameters['xaxisflip'])+'_Td'+str(parameters['Tdust_eq_Thydro'])
    else:
        if parameters['widthkms'] == 0.0:
            parameters['label'] = dir+'_o'+str(parameters['on'])+'_gas'+str(parameters['gasspecies'])+'_iline'+str(parameters['iline'])+'_lmode'+str(parameters['lines_mode'])+'_ab'+str('{:.0e}'.format(
                parameters['abundance']))+'_vkms'+str(parameters['vkms'])+'_turbvel'+str(parameters['turbvel'])+'_nc'+str(parameters['nsec'])+'_xf'+str(parameters['xaxisflip'])+'_Td'+str(parameters['Tdust_eq_Thydro'])
        else:
            parameters['label'] = dir+'_o'+str(parameters['on'])+'_gas'+str(parameters['gasspecies'])+'_iline'+str(parameters['iline'])+'_lmode'+str(parameters['lines_mode'])+'_ab'+str('{:.0e}'.format(
                parameters['abundance']))+'_widthkms'+str(parameters['widthkms'])+'_turbvel'+str(parameters['turbvel'])+'_nc'+str(parameters['nsec'])+'_xf'+str(parameters['xaxisflip'])+'_Td'+str(parameters['Tdust_eq_Thydro'])
