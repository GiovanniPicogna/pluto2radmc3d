...
def compute_opac_mie(
    ...
    assert (
        wavmic.size > 1
    ), "Error: Optical constants file must have at least two rows with two different wavelengths."
    assert (
        wavmic[1] != wavmic[0]
    ), "Error: Optical constants file must have at least two rows with two different wavelengths."
    #
    ...
    if extrapolate:
        ...
        assert np.min(lamcm) >= np.min(
            wavmic * 1e-4
        ), "Error: wavelength range out of range of the optical constants file.\n"
        assert np.max(lamcm) <= np.max(
            wavmic * 1e-4
        ), "Error: wavelength range out of range of the optical constants file.\n"
    #
    # Interpolate
    ...
    #
    if theta is None:
        angles = np.array([0.0, 90.0, 180.0])  # Minimalistic angular grid
    ...
    if angles[0] == 0.0:
        assert (
            angles[nang - 1] == 180
        ), "Error: Angle grid must extend from 0 to 180 degrees."
    else:
    ...
    if logawidth is None:
        ...
        agr = np.exp(
            np.linspace(
                math.log(agraincm) - wfact * logawidth,
                math.log(agraincm) + wfact * logawidth,
                na,
            )
        )
    ...
    if error:
        print(
            "Warning: Angular integral of Z11 is not equal to kscat at all wavelength. "
        )
        print("Maximum error = %13.6e" % errmax)
        if chopforward > 0:
            print(
                "But I am using chopforward to remove strong forward scattering, and then renormalized kapscat."
            )
    #
...
def write_radmc3d_scatmat_file(package, name):
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            "# using the bhmie.py Mie code of Bohren and Huffman (python version by Cornelis Dullemond, from original bhmie.f code by Bruce Draine)\n"
        )
