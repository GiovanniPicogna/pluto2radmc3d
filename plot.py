...
def plot_density(nbin, nsec, ncol, nrad, density, rho):
    ...
    Z = np.abs(density.tmed - np.pi / 2.)

    fig, ax = plt.subplots(nrows=2, ncols=math.ceil(nbin/2), sharey=True, sharex=True, figsize=(8., 8.))
    for n in range(nbin):
        if n < nbin/2:
            i = 0
        else:
        ...
        ax[i][j].pcolormesh(X, Z, axidens[n], norm=colors.LogNorm(vmin=1.e-18, vmax=axidens.max()), cmap=mycolormap)
...
def plot_image(nx, cdelt, lbda0, strflux, convolved_intensity, jybeamfileout, parameters):
    # --------------------
