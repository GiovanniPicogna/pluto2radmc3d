...
from image import Gauss_filter
...
def write_radmc3d_script(parameters):
    ...
    command = (
        "radmc3d image lambda "
        + str(parameters["wavelength"] * 1e3)
        + " npix "
        + str(parameters["nbpixels"])
        + " incl "
        + str(parameters["inclination"])
        + " posang "
        + str(parameters["posangle"] + 90.0)
        + " phi "
        + str(parameters["phiangle"])
    )
    if parameters["plot_tau"] == "Yes":
        command = (
            "radmc3d image tracetau lambda "
            + str(parameters["wavelength"] * 1e3)
            + " npix "
            + str(parameters["nbpixels"])
            + " incl "
...
def exportfits(M, parameters):
    ...
    stdev_x = (parameters["bmaj"] / (2.0 * np.sqrt(2.0 * np.log(2.0)))) / mycdelt
    stdev_y = (parameters["bmin"] / (2.0 * np.sqrt(2.0 * np.log(2.0)))) / mycdelt
            ...
            for i in range(nlam):
                ...
                if parameters["add_noise"] == "Yes":
                    ...
                    noise_dev_std_Jy_per_pixel = (
                        parameters["noise_dev_std"] / fluxfactor
                    ) / np.sqrt(
                        0.5 * beam
                    )  # 1D
                    if i == 0:
                        print(
                            "noise_dev_std_Jy_per_pixel = ", noise_dev_std_Jy_per_pixel
                        )
                    # noise array
                ...
                if parameters["plot_tau"] == "No":
                    ...
                    smooth = (
                        Gauss_filter(
                            im, stdev_x, stdev_y, parameters["bpaangle"], Plot=False
                        )
                        + 1e-20
                    )
                ...
                if parameters["moment_order"] > 1:
                    print("moment map of order > 1 not implemented yet, I must exit!")
                    sys.exit()
    ...
    hdu.header["CDELT1"] = float(-1.0 * pixsize_x_deg)
    hdu.header["CDELT2"] = float(pixsize_y_deg)
    hdu.header["LBDAMIC"] = float(lbda0)
    hdu.header["CUNIT1"] = "deg     "
    hdu.header["CUNIT2"] = "deg     "
    hdu.header["CRPIX1"] = float((im_nx + 1.0) / 2.0)
    hdu.header["CRPIX2"] = float((im_ny + 1.0) / 2.0)
    ...
    LOG.write("pixsize " + str(pixsize_x_deg * 3600.0) + "\n")
    ...
    if parameters["polarized_scat"] == "No" and nlam > 1:
        ...
        hdu2.header["CRVAL3"] = float(-parameters["widthkms"])
        hdu2.header["CDELT1"] = float(-1.0 * pixsize_x_deg)
        ...
        hdu2.header["CRPIX1"] = float((im_nx + 1.0) / 2.0)
        hdu2.header["CRPIX2"] = float((im_ny + 1.0) / 2.0)
...
def write_lines(specie, lines_mode):
    ...
    if os.path.isfile(molecular_file) == False:
        ...
        if specie == "cs":
            datafile = "cs@lique"
        command = (
            "curl -O https://home.strw.leidenuniv.nl/~moldata/datafiles/"
            + datafile
            + ".dat"
        )
        os.system(command)
