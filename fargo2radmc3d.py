...
def main():
    ...
    if parameters['RTdust_or_gas'] == 'dust' and parameters['recalc_density'] == 'Yes' and parameters['polarized_scat'] == 'No':
        ...
        if parameters['bin_small_dust'] == 'Yes':
            ...
            dust_cube[0, :, :, imin:imax] = density.data[:, :, imin:imax] * parameters['ratio'] * frac[0] * \
                                            (density.cumass*1e3)/((density.culength*1e2)** 2.)
        ...
        vol = jacob * dr * dphi * dtheta * ((density.culength * 1e2) ** 3)  # ncol, nrad, Nsec
    ...
    if (parameters['recalc_radmc'] == 'Yes' or parameters['recalc_rawfits'] == 'Yes'):
        ...
        if parameters['recalc_rawfits'] == 'No':
            ...
            write_radmc3dinp(incl_dust=parameters['incl_dust'], incl_lines=parameters['incl_lines'],
                             lines_mode=parameters['lines_mode'], nphot_scat=parameters['nb_photons_scat'],
                             nphot=parameters['nb_photons'], rto_style=3, tgas_eq_tdust=parameters['tgas_eq_tdust'],
                             modified_random_walk=1, scattering_mode_max=parameters['scat_mode'],
        ...
        M = RTmodel(distance=parameters['distance'], Lambda=parameters['wavelength']*1e3, label=parameters['label'],
                    line=parameters['gasspecies'], iline=parameters['iline'],
    ...
    if parameters['recalc_fluxmap'] == 'Yes':
        ...
        plot_image(nx, cdelt, lbda0, strflux, convolved_intensity, jybeamfileout, parameters)
