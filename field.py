...
class Mesh:
    ...
    def __init__(self, directory=""):
        ...
        try:
            ...
            last = np.loadtxt(
                fname=directory + "grid.out",
                skiprows=11 + self.nrad - 1,
                usecols=2,
                max_rows=1,
            )
        ...
        self.rmed = (
            2.0
            * (
                domain_rad[1:] * domain_rad[1:] * domain_rad[1:]
                - domain_rad[:-1] * domain_rad[:-1] * domain_rad[:-1]
            )
            / (domain_rad[1:] * domain_rad[1:] - domain_rad[:-1] * domain_rad[:-1])
            / 3.0
        )  # r-center
        ...
        try:
            domain_th = np.loadtxt(
                fname=directory + "grid.out",
                skiprows=10 + 1 + self.nrad + 1,
                usecols=1,
                max_rows=self.ncol,
            )  # radial interfaces of grid cells
            last = np.loadtxt(
                fname=directory + "grid.out",
                skiprows=10 + 1 + self.nrad + self.ncol,
                usecols=2,
                max_rows=1,
            )
        ...
        # azimuth
        # -----
        self.pedge = np.linspace(0.0, 2.0 * np.pi, self.nsec + 1)  # phi-edge
...
class Field(Mesh):
    ...
    def __init__(self, field, parameters, staggered="c", dtype="float64"):
        ...
        Mesh.__init__(
            self, directory=parameters["dir"]
        )  # all Mesh attributes inside Field

        # get units via definitions.h file
        command = (
            'awk " /^#define  UNIT_LENGTH/ " ' + parameters["dir"] + "definitions.h"
        )
        ...
        command = (
            'awk " /^#define  UNIT_DENSITY/ " ' + parameters["dir"] + "definitions.h"
        )
        # check which version of python we're using
        if sys.version_info[0] < 3:  # python 2.X
            buf = subprocess.check_output(command, shell=True)
